...
class PPONet(nn.Module):
    ...
    def evaluate(self, all_obs, action, agent):
        ...
        full_distribution = self.forward(all_obs[:, agent])
        
        mu = full_distribution.mean[:, :-COMM_DIM]
        std = full_distribution.stddev[:, :-COMM_DIM]
        distribution = torch.distributions.Normal(mu, std)
        ...
        # x = action * 2 -1
        # print(action.shape)
        action = action[:, :-COMM_DIM]
        raw_action = 0.5 * torch.log((1 + action + self.epsilon) / (1 - action + self.epsilon)) # tanh
        log_prob = distribution.log_prob(raw_action).sum(dim=-1)
        log_prob -= torch.log(1 - action.pow(2) + self.epsilon).sum(dim=-1)
    ...
    def get_policy_distribution(self, obs):
        ...
        comm_mu = distribution.mean[:, -COMM_DIM:]
        comm_std = distribution.stddev[:, -COMM_DIM:]
        comm_dist = torch.distributions.Normal(comm_mu, comm_std)
        return comm_dist
...
class PPOAgent:
    ...
    def update(self):
        ...
        # (max_steps, N_agent, obs_dim)
        all_obs = torch.stack([obs[agent] for agent in range(self.n_agents)], dim = 1)
        all_next_obs = torch.stack([next_obs[agent] for agent in range(self.n_agents)], dim = 1)
        ...
        for _ in range(self.ppo_epoches):
            ...
            for agent in range(self.n_agents):
                ...
                # V: from critic; log_prob, entropy: from actor

                # # get new comm
                # # (max_steps, N_agent, act_dim)
                # sample_actions_all = []
                # for j in range(self.n_agents):
                #     sample_actions = self.model.get_act(all_obs[:, j])[2]
                #     if agent != j:
                #         sample_actions = sample_actions.detach()
                #     sample_actions_all.append(sample_actions)
                # sample_actions_all = torch.stack(sample_actions_all, dim=1)

                # # get comm-replaced next_obs
                # # (max_steps, N_agent, obs_dim)
                # comm_next_obs_all = []
                # for j in range(self.n_agents):
                #     comm_next_obs_all.append(torch.cat((all_next_obs[:,j,:-COMM_DIM], sample_actions_all[:,agent,-COMM_DIM:]), dim=-1))
                # comm_next_obs_all = torch.stack(comm_next_obs_all, dim=1)

                # # use new next_obs to get new next_action
                # # (max_steps, N_agent, act_dim)
                # comm_sample_actions_all = []
                # for j in range(self.n_agents):
                #     sample_actions = self.model.get_act(comm_next_obs_all[:, j])[2]
                #     comm_sample_actions_all.append(sample_actions)
                # comm_sample_actions_all = torch.stack(comm_sample_actions_all, dim=1)

                # # Get new log_prob, entropy from comm-replaced next_obs, new next_action
                # log_prob, entropy = torch.zeros_like(log_prob), torch.zeros_like(entropy)
                # for j in range(self.n_agents):
                #     _, new_log_prob, new_entropy = self.model.evaluate(comm_next_obs_all, comm_sample_actions_all[:, j], j)
                #     log_prob += new_log_prob
                #     entropy += new_entropy
                # log_prob /= self.n_agents
                ...
                ppo_loss = actor_loss + self.c1 * critic_loss - self.c2 * entropy.mean()
                ...
                total_losses[agent].append(ppo_loss.item())
                ...
                # === Comm Loss: Entropy Minimization for Receivers ===
                # comm_entropy_loss = 0.0
                # for receiver_id in range(self.n_agents):
                #     if receiver_id == agent:
                #         continue  # don't include self

                #     # Sender (this agent)'s comm
                #     sender_comm = self.model.get_act(all_obs[:, agent])[2][:, -COMM_DIM:]  # (T, comm_dim)

                #     # Receiver's obs with sender's comm
                #     receiver_obs = all_obs[:, receiver_id]
                #     comm_replaced_obs = torch.cat([receiver_obs[:, :-COMM_DIM], sender_comm.detach()], dim=-1)

                #     # Get receiver's policy distribution
                #     dist = self.model.get_policy_distribution(comm_replaced_obs)  # Categorical or Gaussian
                #     comm_entropy_loss += dist.entropy().mean()

                # comm_entropy_loss /= (self.n_agents - 1)  # average over receivers
                # # total_loss = ppo_loss + self.comm_coeff * comm_entropy_loss

                actor_all_loss = actor_loss - self.c2 * entropy.mean() #+ self.comm_coeff * comm_entropy_loss
    ...
    for epoch in tqdm(range(epoches)):
        ...
        for _ in range(max_steps):
            with torch.no_grad():
                ...
                value_n = [torch.tensor(agent.model.evaluate(obs_input, actions_n[i].unsqueeze(0), i)[0].item()).to(device) for i in range(N_AGENTS)]
        ...
        if epoch % check_peroid == 0:
            ...
            if sum(eval_returns) > sum(max_returns):
                ...
                torch.save(agent.model.state_dict(), f"PPO_agent_broadcast_critic.pth")
