...
from train_ppo_share_weight import PPONet
import time
...
class PPOAgent:
    def __init__(self, env, device):
        ...
        for i in range(self.n_agents):
            ...
            self.model[i].load_state_dict(torch.load(f"PPO_agent_{N_AGENTS}_agent.pth"))  # # _broadcast
            self.model[i].eval()
...
print(total_reward)
frames[0].save(f"test_{N_AGENTS}.gif", save_all=True, append_images=frames[1:], optimize=True, loop=0) #_broadcast